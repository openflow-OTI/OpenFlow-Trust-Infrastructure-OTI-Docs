...
### BF21 — TON Jetton Holdings Inferred From Outgoing Messages, Not a Direct Balance Query 🟡 FIX REPORTED — PENDING FRESH VERIFICATION
...
**Fix reported, July 13, 2026:** `getTonJettonBalances()` now calls tonapi.io `/v2/accounts/{addr}/jettons`, filters `is_scam`/zero-balance entries. Reported evidence on `EQB_l_Nt2j997cnhL4wJxPief7wXUnqz9n3kw7FVKXpW5L0A`: 54 real non-spam balances, Railway `uniqueTokens: 54` — exact match. **Not yet closed** — this was the one item in the batch verified against a *cached* Railway response rather than fresh, breaking from the methodology used everywhere else in the batch. Needs a fresh (non-cached) re-check before this is marked done.
