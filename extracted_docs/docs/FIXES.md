...
### BF30 — Bitcoin P2WSH Address Format Rejected by Validator 🟡 IN PROGRESS — UNCONFIRMED
...
**Status as of July 13, 2026:** Backend found a real, currently-valid P2WSH address and began testing it against the updated validator, but hit its daily quota limit before reporting a pass/fail. `bitcoin.ts` is in the pushed commit, so a code change exists, but this item is NOT confirmed done — the new Builder must explicitly re-test and report evidence before this is marked closed. Do not assume it works just because the file was touched.
