...
### BF40 — Dashboard FLAGGED WALLETS Count Mismatches WOR Compromised View ✅ CLOSED (no code change needed) — July 15, 2026
...
**New Backend Builder's verification, July 15, 2026:** psql `SELECT COUNT(*) FROM compromised_wallets` = 1; psql `SELECT COUNT(*) FROM wallet_ownership WHERE status='compromised'` = 0. curl `GET /api/admin/stats` → `total_compromised: 1`. curl `GET /admin/wor/compromised` → `total: 1`, same address as the compromised_wallets row. Since `/api/admin/stats` already reports 1 (not 0), it is not reading from `wallet_ownership` as originally suspected — BF39's fix already pointed the stats handler (or it was already correct) at `compromised_wallets`. No code change deployed. **Not yet independently re-verified by Manager against a second/fresh wallet** — Manager asked Builder for the exact production wallet address behind both counts before treating this as fully closed per D16.
