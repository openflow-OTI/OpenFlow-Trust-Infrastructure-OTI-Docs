...
### BF33 — Smart Contract Diversity `to=self` Bug Also Suspected on TON and Sui 🟡 PARTIALLY VERIFIED — HIGH
...
**Manager live verification (July 13, 2026), signal-by-signal sweep across 11 chains:** Solana, TON, and Sui confirmed fixed on live score cards — real varying counts (2, 92, 2) for wallets outside Backend's own test set. **Tron NOT confirmed** — live score card for wallet `TVj7RNVH...dNZS` still shows "1 smart-contract txns, 20/20", the exact pre-fix fingerprint, on a wallet Backend did not test. Sent back to Backend to re-verify this specific wallet before closing. Not closing BF33 until Tron is confirmed either genuinely low-activity or actually fixed.
