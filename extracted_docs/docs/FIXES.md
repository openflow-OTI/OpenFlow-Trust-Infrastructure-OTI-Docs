...
### BF36 — Contract Addresses Are Scoreable as if They Were Wallets 🔴 OPEN — LOW (needs Ahmad's product call)
**Priority:** Low, product-decision item, not a bug in the scoring math itself. Surfaced during BF33 re-investigation, July 13, 2026. `TVj7RNVHy6thbM7BWdSe9G6gXwKhjhdNZS` (the KLV token's own smart contract, `accountType: 2` per Tronscan) can be entered into the scorer like any wallet address and returns a full trust score. A contract's transaction history is inbound calls made to it, not outbound activity of its own — its scores (e.g. wallet age, contract diversity) don't mean the same thing for a contract as for an EOA. Needs Ahmad to decide: reject contract addresses outright (validation-layer check), label them distinctly in the response so users know what they're looking at, or leave as-is. Not assigned to a Builder yet.
