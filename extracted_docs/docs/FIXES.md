...
### BF36 — Contract Addresses Must Be Rejected, Not Scored 🔴 OPEN — MEDIUM
**Priority:** Medium. Surfaced during BF33 re-investigation, July 13, 2026. `TVj7RNVHy6thbM7BWdSe9G6gXwKhjhdNZS` (the KLV token's own smart contract, `accountType: 2` per Tronscan) can be entered into the scorer like any wallet address and returns a full trust score. A contract's transaction history is inbound calls made to it, not outbound activity of its own — its scores (e.g. wallet age, contract diversity) don't mean the same thing for a contract as for an EOA.
**Decision (Ahmad, July 13, 2026):** No contract address should ever receive a score. OTI scores real people/wallets (EOAs) only. This is not a labeling problem — reject contract addresses outright, the same way an invalid address format is rejected today.
**Fix scope, per chain family (needs a real on-chain check per chain, not a shared heuristic):**
- **Tron:** already confirmed detectable — Tronscan's account endpoint returns `accountType: 2` for contracts vs. normal EOA accounts. Use this at validation/pre-score time.
- **EVM (Ethereum, Polygon, Avalanche, Linea, zkSync, Sonic):** use `eth_getCode` (via Etherscan or direct RPC) — a non-`"0x"` result means it's a contract, reject.
- **Solana:** `getAccountInfo` returns an `executable` flag — `true` means it's a program, reject.
- **Sui:** confirm whether a "package ID" (published Move code) can even be entered in the same address format as a normal account address, or whether they're already structurally distinct — needs investigation, may be a non-issue on Sui.
- **TON:** genuinely harder — on TON, every user wallet IS technically a smart contract (wallet v3/v4/highload contracts). The distinction needed is "known wallet-contract code hash" vs. "arbitrary/application contract" (e.g. a Jetton or DEX contract), not "is it a contract" — needs research into TON's contract code-hash conventions before a check can be written.
**Definition of done:** submitting any of the address types above (contract on each supported chain) returns a clear rejection (e.g. 400 "This address is a smart contract, not a wallet — OTI only scores individual wallets") instead of a score. Not yet assigned to a Builder — queued behind current active work.

### BF35 — "Internal Txns" Label Misleading on Non-EVM Chains (Timing Patterns Signal) 🔴 OPEN — LOW
...
### BF34 — TON Wallet Age Returns 0 Days for Wallets With Real Transaction History 🔴 OPEN — HIGH
