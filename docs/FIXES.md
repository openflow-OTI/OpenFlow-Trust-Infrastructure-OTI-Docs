...
### BF12 — Railway Deploys Don't Auto-Run Migrations ✅
**Fixed:** July 14, 2026. `railway.json` `buildCommand` updated to append `pnpm --filter @workspace/db run push` (drizzle-kit push) after the build step. Plain `push` used deliberately — not `push-force`. Plain push goes through cleanly for additive changes (new tables/columns); for ambiguous/destructive changes it hangs/fails the build rather than silently applying something wrong, which is the correct production behavior. `nixpacks.toml` was not touched. Confirmed correct by Manager July 14, 2026.
...
### BF38 — WOR: Admin Compromised View + FLAGGED WALLETS Card Returning 0 ✅ — July 15, 2026
...
### BF39 — Admin WOR Compromised View Misses Wallets Not in wallet_ownership ⏳ AWAITING AHMAD LIVE VERIFICATION — July 15, 2026
...
### BF40 — Dashboard FLAGGED WALLETS Count Mismatches WOR Compromised View 🔴 ACTIVE — reopened July 15, 2026
...
**New Backend Builder's self-report, July 15, 2026 (claimed no code change needed):** psql `SELECT COUNT(*) FROM compromised_wallets` = 1; curl `GET /api/admin/stats` → `total_compromised: 1`; curl `GET /admin/wor/compromised` → `total: 1`. Wallet address given: `0xd4A1FAC47f8150FF58285D4Ba1a9694b503b2c3F` (EVM).
**Ahmad's live verification, July 15, 2026 (contradicts the Builder's report):** Dashboard shows FLAGGED WALLETS = 1. WOR admin → Compromised tab shows "No registrations found." / 0 results — the flagged wallet is NOT visible there, and Ahmad cannot find it anywhere in the WOR admin section. This is the original bug, still present live. **Reopened — the Builder's curl/psql self-report did not match reality in the browser.** Possible causes to investigate: the `/admin/wor/compromised` endpoint the frontend actually calls may differ from what the Builder curled (auth/session, query params, pagination, environment); or the Builder tested against a different environment/DB than what the live site (otiscore.vercel.app) is reading from. Builder must reproduce this exact discrepancy live, not just via isolated psql/curl, before claiming a fix or a non-issue.

**Root cause narrowed, July 15, 2026 (Builder correction — not a fix yet):** Builder's earlier report only curled one of two relevant endpoints and should not have called them "matching." Two endpoints exist for the Compromised view and disagree on this wallet:
- `GET /admin/wor/compromised` (BF39's dedicated endpoint, reads `compromised_wallets`) → `total: 1`, includes `0xd4A1FAC47f8150FF58285D4Ba1a9694b503b2c3F`.
- `GET /admin/wor/registrations?status=compromised` (generic endpoint, filters only on `wallet_ownership.status`) → `total: 0`, since this wallet's `wallet_ownership.status` is `"active"`, not `"compromised"`.
The second response (`{"registrations":[],"total":0}`) matches the exact "No registrations found." shape Ahmad saw live — so whichever of the two endpoints the deployed frontend's Compromised tab actually calls is the one returning 0. Builder has no access to the frontend repo from their workspace (otiscore.vercel.app is a separate deployment) and cannot inspect its network calls/route wiring to confirm which endpoint is wired up.

**Confirmed root cause, July 15, 2026 (Frontend Builder, code lookup, no changes made):** `src/pages/admin/WOR.tsx` `RegistryTable` component only calls `GET /admin/wor/registrations?status=compromised&...` — there is no frontend call to `/admin/wor/compromised` at all. BF39's dedicated endpoint (sourced from `compromised_wallets`) is unused by the UI. The generic `/admin/wor/registrations?status=compromised` route filters on `wallet_ownership.status = 'compromised'` only — this wallet's `wallet_ownership.status` is `"active"`, so it's invisible to the tab even though it's correctly in `compromised_wallets` and correctly counted by the dashboard stats.
**Fix directive (backend only, no frontend change needed):** make `GET /admin/wor/registrations` — when called with `status=compromised` — source from `compromised_wallets` (LEFT JOIN `wallet_ownership` for metadata), i.e. reuse BF39's query logic, instead of filtering `wallet_ownership.status`. This preserves the single-endpoint contract the frontend already uses and keeps `compromised_wallets` as the one source of truth for every flagged-wallet view (per the standing rule).

**Fix applied, deployed, and closed ✅ — July 15, 2026.** `worAdmin.ts` `GET /admin/wor/registrations`, when `status=compromised`, now sources from `compromised_wallets` LEFT JOIN `wallet_ownership` (BF39's query), instead of filtering `wallet_ownership.status`. Every other status value (active/unset/search) untouched. No frontend change needed. Builder's dev-DB reproduction: before fix, `status=compromised` returned `{"registrations":[],"total":0}` for a `compromised_wallets` row with `wallet_ownership.status="active"` (the exact live bug shape); after fix, `total: 1` with the wallet included; `status=active` and unfiltered both unaffected; `/api/admin/stats` total_compromised matched. Deployed to Railway. **Ahmad tested live and confirmed everything working normally** — dashboard flagged count, WOR admin Compromised tab, and stats all now agree on the one flagged wallet (`0xd4A1FAC47f8150FF58285D4Ba1a9694b503b2c3F`). Closed.

### BF41 — Sui Scoring Broken: JSON-RPC Deprecated (July 27, 2026) 🔴 URGENT
**Priority:** Critical — breaks the week of July 27, 2026 (confirmed by Mysten Labs official docs).
**Discovered:** July 26, 2026. Mysten Labs announced JSON-RPC is fully deprecated and will be disabled the week of July 27, 2026. OTI's entire Sui scoring pipeline (`getSuiTransactions`, `getSuiCoins`, `getSuiWalletAge` etc.) uses the JSON-RPC API (`suix_queryTransactionBlocks`, `suix_getAllCoins`, `suix_getOwnedObjects`). Once disabled, every `?chain=sui` request will error out completely.
**Fix scope:**
- Migrate Sui data-fetching layer from JSON-RPC to Sui's new **gRPC** or **GraphQL RPC** (`/graphql`) endpoints — both are already live and the official replacement paths.
- The Sui GraphQL endpoint (`https://sui-mainnet.mystenlabs.com/graphql`) is the simplest migration path — it replaces `suix_*` methods with GraphQL queries and is accessible without a paid key at moderate rate limits.
- For production scale, a paid Sui RPC provider is required: **Shinami** (free tier: 40 QPS) or **Triton One** — both support the new gRPC/GraphQL interfaces. Free public JSON-RPC endpoint is gone; free public GraphQL endpoint has lower rate limits.
- Sacred files — do not touch while fixing: `src/lib/scoring.ts`, `nixpacks.toml`.
**Evidence required to close:** Builder migrates Sui fetching to GraphQL or gRPC. Fresh live score on a real Sui address (use `0xac5bceec1b789ff840d7d4e6ce4ce61c90d190a7f8c4f4ddf0bff6ee2413c33c` — previously verified at 91/100) returns HTTP 200 with real data after the JSON-RPC shutdown. Builder pastes raw response.

### FF25 — WOR Portal: Register Should Be Primary Entry Point ✅ — July 15, 2026
...
**Builder's report:** no code change needed — Landing.tsx's WOR section, Home.tsx's quiet links, and the results-page CTA already lead with Register; the Report CTA on the results page only shows for an already-registered wallet, so there's no "wrong order" case. Plausible and no risk in shipping, but is a "nothing to fix" claim — worth a quick visual spot-check by Ahmad on the actual pages listed, not closing purely on the Builder's read of their own code.

### FF26 — WOR DApp Connect: OTI Logo Missing (Shows "o" Placeholder) ✅ — July 15, 2026
...
**Builder's report:** exported logo512.png/logo192.png, added manifest.json, updated index.html (manifest link, PNG favicon/apple-touch-icon, corrected theme-color). Notes the app has no WalletConnect/wagmi client at all (custom EIP-6963 + deep-link connector, no WalletConnect session) so the spec's "WalletConnect metadata.icons" step doesn't apply — relying on Trust Wallet reading the page's own icon/manifest tags instead. Builder explicitly flags this **cannot be verified without a live deploy + a real wallet app** (Trust Wallet or similar) opening the connect modal. Not closeable until Ahmad does that live check; if the "o" placeholder persists, Builder's own next guess is a stale favicon/CDN cache rather than more code.

### FF27 — /register Success Screen: OTI Logo in Circle ✅ — July 15, 2026
...
**Builder's report:** no code change needed — success screen already shows the OTI logo inside a mint-bordered circular badge; confirmed by forcing the success state locally and screenshotting (screenshot not shared with Manager). Ahmad should spot-check the real /register flow through to the success screen to confirm before closing.

**ID collision note, July 15, 2026:** Builder's own FIXES.md copy already had FF25/FF26/FF27 assigned to different, earlier-completed work (dashboard scorer move, Query History chain filter, Query History score filter) and reused the same numbers for this round instead of taking new ones. Manager's FIXES.md (source of truth for numbering) has FF25/26/27 correctly assigned to this round's work — no collision here. Builder instructed to renumber the *old* entries in their local copy to the next free IDs (FF28/FF29/FF30) and leave this round's work under FF25/26/27 to match the Manager's file.
